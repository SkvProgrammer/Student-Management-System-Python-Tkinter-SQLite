...
def deletebutton():
    ...
    connection.execute("DELETE " + TABLE_NAME + " ( " + STUDENT_NAME + ", " +
                       STUDENT_COLLEGE + ", " + STUDENT_ADDRESS + ", " +
                       STUDENT_PHONE + " ) VALUES ( '"
                       + username + "', '" + collegeName + "', '" +
                       address + "', " + str(phone) + " ); ")
...
button = tk.Button(root, text="Add Info",font = ('calibri', 10, 'bold'),command=lambda :takeNameInput())
...
deletebutton = tk.Button(root, text="Delete Info",font = ('calibri', 10, 'bold'),command=deletebutton())
deletebutton.grid(row=6, column=0, pady=30)
root.mainloop()
